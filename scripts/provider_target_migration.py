...
from s3mp.files.infrastructure.models import (
    ...
    ProviderMigrationManifestModel,
    UploadSessionModel,
)
...
async def plan(session_factory: async_sessionmaker[Any]) -> list[ProviderMigrationPlan]:
    async with session_factory() as session:
        ...
        async def append_rows(model: Any, record_type: str, *, ingestion: bool = False) -> None:
            for row in (
                await session.scalars(select(model).where(model.provider_target_version == 0))
            ).all():
                ...
                plans.append(
                    classify_legacy_target(
                        ...
                        storage_space=spaces.get(row.storage_space_id),
        ...
        for row in (
            await session.scalars(
                select(FileOperationModel).where(FileOperationModel.provider_target_version == 0)
            )
        ).all():
            plans.append(
                classify_legacy_target(
...
async def record_manifests(
    session_factory: async_sessionmaker[Any], plans: list[ProviderMigrationPlan]
) -> None:
    async with session_factory.begin() as session:
        for item in plans:
            existing = await session.scalar(
                select(ProviderMigrationManifestModel).where(
...
async def run(*, persist: bool = False) -> dict[str, Any]:
    ...
    try:
        ...
        return {
            "mode": "manifest_recorded" if persist else "dry_run",
            "records": len(plans),
...
async def execute(*, cleanup: bool = False, limit: int = 100) -> dict[str, int]:
    ...
    try:
        ...
        result = await (
            executor.cleanup_verified_sources(limit) if cleanup else executor.copy_verified(limit)
        )
        return {
...
def main() -> None:
    ...
    parser.add_argument(
        "--record-manifests",
        action="store_true",
        help="persist plans only; never moves provider objects",
    )
    parser.add_argument(
        "--apply-verified-copy",
        action="store_true",
        help="copy only reviewed executable manifests and retain old objects",
    )
    parser.add_argument(
        "--cleanup-verified-sources",
        action="store_true",
        help="delete old objects only after a verified copy",
    )
    ...
    if args.apply_verified_copy or args.cleanup_verified_sources:
        print(
            json.dumps(
                asyncio.run(execute(cleanup=args.cleanup_verified_sources, limit=args.limit)),
                sort_keys=True,
            )
        )
        return
