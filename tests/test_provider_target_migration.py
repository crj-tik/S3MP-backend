...
def test_legacy_ingestion_with_proven_relative_key_is_ready_for_verified_copy() -> None:
    ...
    plan = classify_legacy_target(
        ...
        record_id=record_id,
        tenant_id=tenant_id,
        storage_space=_space(tenant_id, space_id),
        source_bucket="s3mp-dev",
        source_key="legacy/report.csv",
        relative_key="report.csv",
        overlapping_space_ids=set(),
    )
...
def test_overlapping_legacy_roots_are_quarantined_for_every_fixture_kind() -> None:
    ...
    for record_type in (
        ...
        plan = classify_legacy_target(
            ...
            storage_space=_space(tenant_id, space_id),
            source_bucket="s3mp-dev",
            source_key="legacy/a.txt",
            relative_key="a.txt",
            overlapping_space_ids={space_id},
        )
