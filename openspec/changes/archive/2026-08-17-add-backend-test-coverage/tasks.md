...
- [x] 0.1 修正 `deploy/secrets/database_url` 为 `postgresql+asyncpg://s3mp_app:bk-s3mp-backend@localhost:18110/s3mp`（当前 `s3mp` 用户不存在，与 `alembic.ini` 一致）。验证：`cat deploy/secrets/database_url`
