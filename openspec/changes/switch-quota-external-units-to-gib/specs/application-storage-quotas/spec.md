...
系统 SHALL 支持租户总配额和应用配额；平台配额管理请求 SHALL 使用 `limit_gib`，响应 SHALL 返回 GiB 展示字段以及可选的精确 bytes 诊断字段。应用配额不得超过租户可分配容量，内部校验 SHALL 基于整数 bytes 执行；租户和应用的响应 SHALL 分别返回 limit、used、reserved 和 available 等可计算字段。

#### Scenario: Application quota exceeds tenant quota

- **WHEN** 管理员为应用设置大于租户剩余可分配容量的 `limit_gib`
- **THEN** 系统 SHALL 拒绝配置并返回稳定的配额校验错误

#### Scenario: Usage is queried

- **WHEN** 管理员查询租户或应用的容量状态
- **THEN** 系统 SHALL 返回当前已确认使用量、进行中预留量和可用量的 GiB 展示值，并明确统计时间或一致性状态

#### Scenario: Public byte input is removed

- **WHEN** 客户端提交旧的 `limit_bytes` 而不提交 `limit_gib`
- **THEN** 系统 SHALL 按契约拒绝该请求，不得把旧字段静默解释为 GiB

### Requirement: Atomic upload reservation

创建上传或 multipart 会话时，系统 SHALL 在同一事务边界内以内部 bytes 检查租户和应用剩余容量并原子增加 reservation；并发请求不得共同通过同一剩余容量检查。
