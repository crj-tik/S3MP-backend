...
def upgrade() -> None:
    ...
    op.add_column(
        "file_object",
        sa.Column("deletion_next_retry_at", sa.DateTime(timezone=True), nullable=True),
    )
    op.add_column(
        "file_object", sa.Column("deletion_failure_reason", sa.String(128), nullable=True)
    )
    op.add_column("file_object", sa.Column("deletion_principal_id", sa.Uuid(), nullable=True))
    op.add_column(
        "file_object", sa.Column("deletion_authorization_version", sa.Integer(), nullable=True)
    )
    op.add_column(
        "file_object", sa.Column("deletion_authorization_evidence", sa.JSON(), nullable=True)
    )
